...
// Categorias são compartilhadas entre Compras e Estoque via API (/api/config).
...
export default function Servicos({ orders = [] }) {
  ...
  // Categorias vindas da API — acessíveis em qualquer dispositivo
  const { config, updateConfig } = useConfig();
  const categorias    = config.compras_categorias ?? [];
  const setCategorias = (val) => updateConfig('compras_categorias', val);
